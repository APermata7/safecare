{{-- File: resources/views/profile/partials/panti-fields.blade.php --}}
...
<div>
    ...
    <textarea id="alamat" name="alamat" rows="3" class="mt-1 block w-full border-gray-300 focus:border-primary-green focus:ring-primary-green focus:ring-opacity-50 rounded-xl shadow-sm" required>{{ old('alamat', $pantiAsuhan->alamat ?? '') }}</textarea>
    <x-input-error class="mt-2" :messages="$errors->get('alamat')" />
</div>
<div>
    <x-input-label for="deskripsi" :value="__('Description')" />
    <textarea id="alamat" name="alamat" rows="3" class="mt-1 block w-full border-gray-300 focus:border-primary-green focus:ring-primary-green focus:ring-opacity-50 rounded-xl shadow-sm" required>{{ old('deskripsi', $pantiAsuhan->deskripsi ?? '') }}</textarea>
    <x-input-error class="mt-2" :messages="$errors->get('deskripsi')" />
</div>
<div>
    <x-input-label for="foto_profil_panti" :value="__('Orphanage Profile Photo (Banner)')" />
    <input id="foto_profil_panti" name="foto_profil" type="file" class="mt-1 block w-full text-sm text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-gray-100 file:text-primary-green hover:file:bg-gray-200" accept="image/*" />
...
<div>
    ...
    <input id="dokumen_verifikasi" name="dokumen_verifikasi" type="file" class="mt-1 block w-full text-sm text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-gray-100 file:text-primary-green hover:file:bg-gray-200" accept=".pdf,.jpg,.jpeg,.png" />
